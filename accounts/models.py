...
class Profile(models.Model):
    ...
    def is_otp_valid(self, otp_code):
        ...
        return False
    def verify_and_delete_otp(self, otp_code):
        """
        Verifies the OTP and deletes it if valid.
        """
        if self.is_otp_valid(otp_code):
            self.otp = None
            self.otp_created_at = None
            self.save()
            return True  # OTP valid and deleted
    ...
    def delete_expired_otps():
        ...
        Profile.objects.filter(otp_created_at__lt=expired_time).update(otp=None, otp_created_at=None)
